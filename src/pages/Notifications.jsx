...
import { notificationsPageQueryKey } from "../api/queryKeys";
import {
  resolveFollowStateFromResponse,
...
const loadNotificationsPage = async () => {
  ...
  const list =
    notifResult.status === "fulfilled" && Array.isArray(notifResult.value?.data)
      ? notifResult.value.data
      : [];
  const followRequests =
    followResult.status === "fulfilled" && Array.isArray(followResult.value)
      ? followResult.value
      : [];
  ...
  merged.forEach((entry) => {
    ...
    if (extracted === true) {
      followedById[entry?.id] = true;
      updateFollowCache(identifiers, true);
      return;
    }
    if (extracted === false) {
      followedById[entry?.id] = false;
      return;
    }
  ...
  return { items: merged, followedById };
};
...
export default function Notifications() {
  ...
  useEffect(() => {
    if (notificationsQuery.data) {
      const nextItems = Array.isArray(notificationsQuery.data.items)
        ? notificationsQuery.data.items
        : [];
      setItems(nextItems);
      writeNotificationsCache(nextItems);
      setFollowedById(notificationsQuery.data.followedById || {});
      return;
    }
  ...
  const loading = notificationsQuery.isPending && items.length === 0;
  ...
  const handleFollow = async (item) => {
    ...
    try {
      ...
      if (nextState === "following") {
        ...
        setRequestedById((prev) => ({ ...prev, [id]: false }));
        syncCurrentViewerFollowRelation({
          targetIdentifiers: [
        ...
        setRequestedById((prev) => ({ ...prev, [id]: true }));
        syncCurrentViewerFollowRelation({
          targetIdentifiers: [
            item?.actorIdentifier,
            item?.actorEmail,
      ...
      void queryClient.invalidateQueries({ queryKey: notificationsPageQueryKey() });
  ...
  const acceptFollowRequest = async (requestId) => {
    ...
    try {
      ...
      const requestItem = items.find((entry) => String(entry?.followRequestId || "") === idText);
      syncAcceptedFollowRequest({
        requesterIdentifiers: [
          requestItem?.actorIdentifier,
          requestItem?.actorEmail,
          requestItem?.actorUsername,
          requestItem?.actor?.id,
          requestItem?.actor?.email,
          requestItem?.actor?.username
        ]
      });
    ...
    <section className="notify-page">
      <header className="notify-head">
        <div className="notify-brand">
          <img src="/logo-clean-round.png?v=1" alt="SocialSea" className="notify-logo" />
          <div className="notify-brand-copy">
            <h2>Notifications</h2>
            <p>{unreadCount} unread alerts</p>
          </div>
        </div>
        <div className="notify-meta" aria-label="Notification summary">
          <div className="notify-stat">
            <span>Total</span>
            <strong>{items.length}</strong>
          </div>
          <div className="notify-stat is-highlight">
            <span>Unread</span>
      ...
      <div className="notify-list">
        ...
        {items.map((n) => {
          ...
          const requested = !!requestedById[n?.id];
                ...
                <div className="notify-row">
                  ...
                  {canFollow ? (
                    <button
                      ...
                      disabled={followBusy || following || requested}
                    >
                      <Icon />
                      {followBusy ? "..." : following ? "Following" : requested ? "Requested" : "Follow"}
                    </button>
                  ) : (
